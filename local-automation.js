...
class LocalAutomationService {
  ...
  async downloadCSVForWebsite(website) {
    ...
    try {
      ...
      // Send CSV file to Hetzner server for processing
      await this.sendCSVToServer(filePath, website);
  ...
  async sendCSVToServer(filePath, website) {
    try {
      console.log(`📤 Sending CSV file to Hetzner server for processing...`);
      ...
      form.append('website', JSON.stringify(website));
      form.append('source', 'local-automation');

      const response = await fetch(`${this.serverUrl}/api/recipes/upload-csv`, {
        method: 'POST',
        body: form,
      ...
      if (response.ok) {
        ...
        console.log('✅ CSV sent to server successfully:', result);
      } else {
        console.error('❌ Failed to send CSV to server:', response.statusText);
      }
    } catch (error) {
      console.error('❌ Error sending CSV to server:', error.message);
    }
  }
